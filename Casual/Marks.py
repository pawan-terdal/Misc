...
for i in range(1, 135):
    ...
    if(len(name) != 0):
        d.setdefault("1PE15IS{0:03d} >>".format(i) + name[0], float(sgpa[0][3:-4]))
...
for i in range(1, 195):
    ...
    sgpa = reg2.findall(text)
    if(len(name) != 0):
        d.setdefault("1PE15CS{0:03d} >>".format(i) + name[0], float(sgpa[0][3:-4]))
...
for i in range(1, 195):
    ...
    sgpa = reg2.findall(text)
    if(len(name) != 0):
        d.setdefault("1PE15EC{0:03d} >>".format(i) + name[0], float(sgpa[0][3:-4]))
...
for i in range(1, 195):
    ...
    sgpa = reg2.findall(text)
    if(len(name) != 0):
        d.setdefault("1PE15ME{0:03d} >>".format(i) + name[0], float(sgpa[0][3:-4]))


count = 1
for key in sorted(d, key=d.__getitem__)[::-1]:
    print(str(count) + "))" + key + ">>>" + str(d[key]))
    count += 1
